<template>
	<view :style="disabled ? 'background: #F0F0F0;' : ''" @click="click" class="back">
		...
		<view class="contentback back1">
			<titleDateTimeSelect v-if="!disabled1" :defaultValue="textvalue" class="input" @change="bindPickerChange">
			</titleDateTimeSelect>
			<view v-else class="contentback"><input disabled v-model="text" :placeholder="placeHolder2"
...
<script>
	import titleDateTimeSelect from './biaofun-datetime-picker/biaofun-datetime-picker.vue';

	export default {
		components: {
			titleDateTimeSelect
		},
		props: ['title', 'text', 'placeHolder', 'disabled', 'tag', 'required'],
		data() {
			return {
				textvalue: this.text,
